PropaneStation_ToolTip = ISPanel:derive("PropaneStation_ToolTip");
function PropaneStation_ToolTip:initialise()
  ISPanel.initialise(self);
end

function PropaneStation_ToolTip:setItem(it)
  self.Item = it
end

function PropaneStation_ToolTip:isItem()
  return self.Item 
end

function PropaneStation_ToolTip:setName(name)
  self.name = name;
end

function PropaneStation_ToolTip:setContextMenu(contextMenu)
  self.contextMenu = contextMenu;
end

function PropaneStation_ToolTip:setTexture(textureName)
  self.texture = getTexture(textureName);
end

function PropaneStation_ToolTip:onMouseDown(x, y)
  return false
end

function PropaneStation_ToolTip:onMouseUp(x, y)
  return false
end

function PropaneStation_ToolTip:onRightMouseDown(x, y)
  return false
end

function PropaneStation_ToolTip:onRightMouseUp(x, y)
  return false
end

function PropaneStation_ToolTip:prerender()
...
function PropaneStation_ToolTip:render()
...
function PropaneStation_ToolTip:doLayout()
  local textX = 10
  local textY = 0
  local textWidth = 0
  local textHeight = 0
...
function PropaneStation_ToolTip:setDesiredPosition(x, y)
  self.desiredX = x
  self.desiredY = y
...
function PropaneStation_ToolTip:adjustPositionToAvoidOverlap(avoidRect)
...
function PropaneStation_ToolTip:overlaps(r1, r2)
  return r1.x + r1.width > r2.x and r1.x < r2.x + r2.width and
      r1.y + r1.height > r2.y and r1.y < r2.y + r2.height
end

function PropaneStation_ToolTip:placeLeft(r1, r2)
  local r = r1
  r.x = math.max(0, r2.x - r.width - 8)
  return r
end

function PropaneStation_ToolTip:placeRight(r1, r2)
  local r = r1
  r.x = r2.x + r2.width + 8
...
function PropaneStation_ToolTip:placeAbove(r1, r2)
  local r = r1
  r.y = r2.y - r.height - 8
  r.y = math.max(0, r.y)
  return r
...
function PropaneStation_ToolTip:setOwner(ui)
  self.owner = ui
end

function PropaneStation_ToolTip:new()
   local o = {}
   --o.data = {}
